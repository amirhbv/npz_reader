...
if [ -f VERSION ]; then
    ...
    cat package.json | jq ".version = \"$(cat VERSION)\"" > package.tmp.json
    ...
    git commit -m "docs: Bump version to ${INPUT_STRING}
    ...
    if [ "$RESPONSE" = "y" ]; then
        echo "0.1.0" > VERSION
        cat package.json | jq ".version = \"0.1.0\"" > package.tmp.json
        mv package.tmp.json package.json
        ...
        git commit -m "docs: Bump version to 0.1.0

        Add VERSION and CHANGELOG.md files."
